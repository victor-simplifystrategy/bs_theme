...
function isMoblieBrowser() {
        ...
        c.extend(h, {
            ...
            scaupdate: function(a) {
                ...
                if (d && (clearTimeout(j), j = null), h.isOpen && !j) {
                    var e, e = 0;
                    e = c("#qv-left").outerHeight() > c(".fancybox-inner").outerHeight() ? 345 : 0, 0 == c("#qv-zoomcontainer").outerHeight() && c("#qv-left").outerHeight() !== c(".fancybox-inner").outerHeight() && (0 == e && (e = 345), c("#qv-zoomcontainer").height(c("#qv-zoomImg").outerHeight())), j = setTimeout(function() {
                        var c = h.current;
                        c && !h.isClosing && (h.wrap.removeClass("fancybox-tmp"), (d || "load" === b || "resize" === b && c.autoResize) && h._setDimension(), "scroll" === b && c.canShrink || h.reposition(a), h.trigger("onUpdate"), j = null, h.hideLoading())
                    }, e)
                }
            },
            toggle: function(a) {
    ...
    function loadData(a, b) {
        ...
        var c = b.jsondata;
        return moneyFormat = b.moneyFormat, console.log("collection_handle : " + collection_handle), noimage = b.noimage, console.log("no image : " + noimage), add_to_cart_text = a(".qv-cartbtn").val(), a("#qv-unavailable").length && (unavailable_text = a("#qv-unavailable").text()), b.overlay_lock && (overlay_lock = b.overlay_lock), settings = a.parseJSON(SCABase64.decode(c)), console.log("Disable QV : " + settings.qv_disable), "yes" == settings.qv_disable ? !1 : (SCASettings.loadSettings(a, settings), !0)
    }

    function createCartForm(a, b, c) {
        var d = a("#qv-add-item-form", c);
        if (0 != d.length) {
            var e = a(".qv-product-options", d),
                f = a("#qv-variant-options", e);
          
            f.empty(), f.append('<div > <select id="qv-product-selected" name="id" class="qv-hidden"> </select> </div>');
            var g = a("#qv-product-selected", f);
            g.empty();
            var h = b.variants,
                i = !1;
            a.each(h, function(a, b) {
                ...
                var c = "";
              // 19-02-2017 added data attr in option  
              b.available && 0 == i && (i = !0, c = 'selected="selected"'), g.append('<option value="' + b.id + '" ' + c + " data="+ b.title +">" + b.title + " - " + SCAShopify.formatMoney(b.price, moneyFormat) + "</option>")
            }), a("#qv-quantity", c).val("1"), a("#qv-sold-out").addClass("qv-hidden"), b.available ? (a(".qv-cartbtn").removeClass("qv-hidden"), a(".qv-cartbtn").removeClass("disabled").removeAttr("disabled").val(add_to_cart_text)) : (a(".qv-cartbtn").removeClass("qv-hidden"), a(".qv-cartbtn").val(a("#qv-sold-out").text()).addClass("disabled").attr("disabled", "disabled")), formatVariantOption(a, b)
        }
    }

    function showIfProductAdd(a, b) {
        -1 != a.inArray(b, list_cart_variants) && (a("#qv-addcart-msg").html(product_in_cart), a("#qv-addcart-msg").fadeIn(1e3))
    }
    ...
    function formatVariantOption(a, b) {
        var c = function(b, c) {
                a("#qv-addcart-msg").hide();
                var d = "";
                void 0 != settings.price_color && null != settings.price_color && (d = settings.price_color);
                var e = a("#qv-showqv");
                b ? (b.available ? (showIfProductAdd(a, b.id), a(".qv-cartbtn").removeClass("disabled").removeAttr("disabled").val(add_to_cart_text)) : a(".qv-cartbtn").val(a("#qv-sold-out").text()).addClass("disabled").attr("disabled", "disabled"), b.compare_at_price > b.price ? a(".qv-price-container", e).html('<span class="qv-product-price" style="color:' + d + ' !important;">' + SCAShopify.formatMoney(b.price, moneyFormat) + '&nbsp;<del class="qv-product-compare-price">' + SCAShopify.formatMoney(b.compare_at_price, moneyFormat) + "</del></span>") : a(".qv-price-container", e).html('<span class="qv-product-price"  style="color:' + d + ' !important;">' + SCAShopify.formatMoney(b.price, moneyFormat) + "</span>")) : a(".qv-cartbtn").val(unavailable_text).addClass("disabled").attr("disabled", "disabled")
            },
            d = a("#qv-showqv");
        try {
            new Shopify.OptionSelectors("qv-product-selected", {
                product: b,
                onVariantSelected: c
            })
        } catch (e) {
        ...
        a.each(g, function(c, e) {
            if (e.available && 0 == f) {
                ...
                a.each(g, function(b, c) {
                    ...
                    a(".single-option-selector:eq(" + b + ")", d).val(e.options[b]).trigger("change")
                })
            }
        }), a("#qv-variant-options").removeClass("qv-hidden"), 1 == b.variants.length && b.variants[0].title.indexOf("Default") > -1 && a("#qv-variant-options").addClass("qv-hidden")
    }
    ...
    function truncate(a, b) {
        ...
        var d = b.split(" "),
            e = 30,
            f = d.length > e,
            b = f ? d.splice(0, e).join(" ") + "..." : b;
        return b
    }

    function createDesc(a, b, c) {
        var d = a("#qv-des", c);
        d.empty(), d.html(b.description), d.text(truncate(a, d.text())), d.append('<div class="clear-both"></div')
    }
    ...
    function createPrice(a, b, c) {
        ...
        var d = a("#qv-price-container", c);
        d.empty(), b.compare_at_price > b.price ? (a("#qv-sale").removeClass("qv-hidden"), d.append('<span class="qv-product-price ">' + SCAShopify.formatMoney(b.price, moneyFormat) + '&nbsp;<del class="qv-product-compare-price">' + SCAShopify.formatMoney(b.compare_at_price, moneyFormat) + "</del>  </span>")) : d.append('<span class="qv-product-price ">' + SCAShopify.formatMoney(b.price, moneyFormat) + "</span>")
    }
    ...
    function createGallery(a, b, c) {
        var d = a("#qv-galleryid", c);
        d.empty();
        var e = b.images,
            f = parseInt(settings.img_main_width, 10),
            g = (f - 8) / 4;
        e.length > 1 && (cratezoomgalery(a, d, b.featured_image, g), a.each(e, function(b, c) {
            b > 0 && 10 > b && cratezoomgalery(a, d, c, g)
        }))
    }

    function cratezoomgalery(a, b, c, d) {
        var e = "max-width:" + d + "px;max-height:" + d + "px;  ",
            f = '<a href="#" data-image="' + SCAShopify.resizeImage(c, "large") + '" data-zoom-image="' + SCAShopify.resizeImage(c, "original") + '"  class="qv-gallery-a" > <img src="' + SCAShopify.resizeImage(c, "large") + '" class="qv-gallery-img" style="' + e + '"/> </a>';
        b.append(f)
    
       //a('#imgswatch').append("<input type='hidden' id='' name='' data-zoom-image='"++ SCAShopify.resizeImage(c, "original")+"' value='"+SCAShopify.resizeImage(c, "large")+"' >");
    }

    function createZoomContainer(a, b, c) {
        var d = a("#qv-zoomcontainer", c);
        d.empty();
        var e = noimage;
        null != b.featured_image && (e = b.featured_image), d.append('<img  id="qv-zoomImg" style="max-width:' + settings.img_main_width + ";max-height:" + settings.img_main_width + '" src="' + SCAShopify.resizeImage(e, "large") + '" data-zoom-image="' + SCAShopify.resizeImage(e, "original") + '" alt=""  >')
    }

    function cleanTemplate(a, b) {
        var c = a("#qv-zoomcontainer", b);
        c.empty();
        var d = a("#qv-galleryid", b);
        d.empty();
        var e = a("#qv-title", b);
        e.empty();
        var f = a("#qv-price-container", b);
        f.empty();
        var g = a("#qv-des", b);
        g.empty();
        var h = a("#qv-add-item-form", b),
            i = a(".qv-product-options", h),
            j = a("#qv-variant-options", i);
        j.empty();
        var k = a("#qv-product-selected", j);
        k.empty()
    }
    ...
    function freegifts_filter(a, b) {
        ...
        return a.each(b.variants, function(b, d) {
            ...
            a.each(d.options, function(a, b) {
                if (b.length > 6 && "% off)" == b.substring(b.length - 6, b.length)) return e = !0, !1;
                if (b.length > 11 && "(Freegifts)" == b.substring(b.length - 11, b.length)) return e = !0, !1;
                try {
                    var c = /\(BK \d+\)/g;
                    if (b.match(c)) return e = !0, !1
                } catch (d) {}
            }), e || c.push(d)
        }), b.variants = c, b
    }
    ...
    function executeFunctionByName(a, b) {
        for (var c = Array.prototype.slice.call(arguments, 2), d = a.split("."), e = d.pop(), f = 0; f < d.length; f++) b = b[d[f]];
        return b[e].apply(b, c)
    }
    "undefined" == typeof SCAShopify && (SCAShopify = {}), SCAShopify.preload = function(a, b) {
    ...
    var loadjscssfile = function(a, b) {
        ...
        SCASettings = {
            loadSettings: function(a, b) {
                a("#qv-cartform .qv-product-options").append('<div id="qv-addcart-msg" class="qv-addcart-msg" style="position: absolute !important; margin-top:15px"></div>'), void 0 != b.cart_notify_add_format && void 0 != b.cart_notify_in_format && (product_in_cart = b.cart_notify_add_format.replace("*", '<a href="/cart"  class="qv-msg-cart-link">').replace("%", "</a>"), product_added = b.cart_notify_in_format.replace("*", '<a href="/cart"  class="qv-msg-cart-link">').replace("%", "</a>"));
                var c = ".fancyox-view-detail .zoomWrapper img {  max-width:" + b.img_main_width + " !important ; max-height:" + b.img_main_width + " !important ;   }",
                    d = document.head || document.getElementsByTagName("head")[0],
                    e = document.createElement("style");
                if (void 0 != b.cart_color) {
                    var f = " a.qv-cartbtn-config, input.qv-cartbtn-config {background:none repeat scroll 0 0 " + b.cart_color + " !important;}";
                    c += f, a(".qv-cartbtn").addClass("qv-cartbtn-config")
                }
                if (void 0 != b.sale_icon_color) {
                    var g = "  .qv-sale-settings {background-color:" + b.sale_icon_color + " !important;}";
                    c += g, a("#qv-sale").addClass("qv-sale-settings")
                }
                if (void 0 != b.link_color) {
                    var g = "  #qv-right a {color:" + b.link_color + ";}";
                    c += g
                }
                if (void 0 != b.title_color) {
                    var g = "  #qv-title strong {color:" + b.title_color + ";}";
                    c += g
                }
                e.type = "text/css", e.styleSheet ? e.styleSheet.cssText = c : e.appendChild(document.createTextNode(c)), d.appendChild(e);
                var h = a(".qv-button"),
                    i = h.parent();
                i.parent().addClass("qv-image"), h.text(b.bnt_text), h.hover(function() {
                    a(this).css({
                        color: b.bnt_text_hover,
                        background: b.bnt_color_hover
                    })
                }, function() {
                    a(this).css({
                        color: b.bnt_text_color,
                        "background-color": b.bnt_color
                    })
                }), h.css({
                    "font-family": "auto" != b.bnt_text_font ? b.bnt_text_font : "",
                    "font-size": b.bnt_text_fontsize,
                    top: "top" == b.bnt_possition ? "20%" : "bottom" == b.bnt_possition ? "80%" : "5%0",
                    color: b.bnt_text_color,
                    "background-color": b.bnt_color
                }), a(".qv-image").hover(function() {
                    a(".qv-button", this).parent().show()
                }, function() {
                    a(".qv-button", this).parent().hide()
                })
            }
        },
        SCABase64 = {
        ...
        settings, moneyFormat, add_to_cart_text, unavailable_text = "Unavailable",
        ...
        initQuickViewContent = function(a) {
            ...
            for (var b = '<div class="qv-button-wrap" style="display: none !important;" ><a class="qv-button"  href="#qv-showqv" handle="?" ></a></div>', c = a('a[href*="/products/"]:not([href*=".jp"]):not(form a[href*="/products/"]):not(.related-products a[href*="/products/"]):not(.not-qv a[href*="/products/"]):not([href*=".JP"]):not([href*=".png"]):not([href*=".PNG"])').has('img[src*="/products/"]:not([class*="not-qv"]), img[src*="/no-image"]'), d = 0; d < c.length; ++d) {
                var e = c.eq(d),
                    f = e.attr("href").split("/"),
                ...
                }(void 0 == h || null == h) && (h = e.parent()), 0 == a(".qv-button", h).length && h.append(b.replace("?", g))
            }
        },
        
        product_in_cart = 'This product in <a href="/cart"  class="qv-msg-cart-link">cart</a> already.',
        product_added = 'Item added to cart! <a href="/cart"  class="qv-msg-cart-link">View Cart</a>.',
        collection_handle, initSettings = function(a) {
            a = SCAShopify.jQuery, loadProductFromCart(a);
          
            var b = !1;
            return a.ajax({
                dataType: "json",
                url: "/index?view=quick-settings",
        ...
        callbackQV = function(a, b) {
            ...
            createQuickViewVariantsSwatch(b,t,a);
         // 19-02-2017
          var cs = [];
            a.each(b.variants, function(a, b) {
            //  alert(b.id + " === "+ b.featured_image);
              var featured_json_obj = JSON.parse(JSON.stringify(b.featured_image));
  			//	console.log(JSON.stringify(b.featured_image));
              $.each(featured_json_obj, function(i, item) {
   					// alert(i + '--' + item);
                if(i == 'src'){
                $('#imgswatch').append("<input type='hidden' id='"+b.id+"' name='' data-zoom-image='"+item+"' value='"+item+"' >");
                }
				});
         ...
         // end 19-02-2017 
            ...
            try {
                b = freegifts_filter(a, b), b = buyx_product_json(a, b)
            } catch (d) {
                console.log(d)
            }
            var e = a("#qv-left");
            createZoomContainer(a, b, e), createGallery(a, b, e), SCAShopify.preload(b.images), SCAShopify.preload(b.images, "large");
            var f = a("#qv-right");
            createTitle(a, b, f), createPrice(a, b, f), createDesc(a, b, f), createDetail(a, b, f, b.handle), createCartForm(a, b, f);
            try {
                try {
                ...
                console.log("custom quick view function error"), console.log(d)
            }
            var g = new Image;
            g.onload = function() {
                var b = g.height,
                    ...
                    d = setInterval(function() {
                        g = new Image, g.src = a("#qv-zoomImg").attr("src"), b = g.height, --c, (0 == c || b > 0) && (window.clearInterval(d), zoomAllProduct(a), a("#qv-zoomcontainer").outerHeight() < a("#qv-zoomImg").outerHeight() && a("#qv-zoomcontainer").height(a("#qv-zoomImg").outerHeight()), "auto" == settings.wd_width || "auto" == settings.wd_height ? a.scafancybox.scaupdate() : (a("#qv-right").outerHeight() > a("#qv-left").outerHeight() && a("#qv-left").outerHeight(a("#qv-right").outerHeight()), a.scafancybox.hideLoading()))
                    }, 300)
            }, g.onerror = function() {
                console.error("Cannot load image"), a.scafancybox.hideLoading()
            }, g.src = a("#qv-zoomImg").attr("src"), a("#qv-right").outerHeight() > a(".fancybox-inner").outerHeight() && (a("#qv-left").outerHeight(""), a("#qv-left").height(""), a("#qv-right").outerHeight(""), a("#qv-right").height(""), a(".fancybox-inner").css("overflow", "auto"));
        ...
        displayQuickView = function(a) {
            ...
            ("auto" == settings.wd_width || "auto" == settings.wd_height) && (b = !0, c = e), "auto" != settings.wd_height && (d = f), a(".qv-button").scafancybox({
                arrows: !1,
                openEffect: "elastic",
                scrolling: "no",
                maxHeight: d,
                ...
                beforeLoad: function() {
                    loadProductFromCart(a), cleanTemplate(a, a("#qv-showqv")), a("#qv-addcart-msg").hide(), a("#qv-sale").addClass("qv-hidden")
                },
                afterShow: function() {
                    ...
                    } else if (a('span[class="qv-handle"]', b.parent()).length > 0) {
                        d = a('span[class="qv-handle"]', b.parent());
        ...
        zoomAllProduct = function(a) {
            function b(a) {
                a("#qv-zoomImg").elevateZoom({
                    gallery: "qv-galleryid",
                    zoomWindowOffetx: 10,
                    borderSize: 1,
                    scrollZoom: !0,
                    cursor: "pointer"
        ...
        onAddCartClicked = function($) {
            function updateCartcount(a) {
                var b = parseInt($('[name="quantity"]', a).val(), 10) || 1,
                    c = parseInt($(settings.cartcount).text(), 10) || 0,
                    d = c + b;
            ...
            $(".qv-cartbtn").click(function(a) {
                if ("no" == settings.cart_go) {
                    var b = $(this),
                        c = $("#qv-add-item-form");
                    $(b).prop("disabled", !0), a.preventDefault(), addItem("qv-add-item-form", c)
                } else {
                    var c = $("#qv-add-item-form");
                    c.attr("action", "/cart/add"), c.submit()
                }
            });
            var scaonSuccess = function(a) {
                    $("#qv-addcart-msg").hide(), $("#qv-addcart-msg").html(product_added), $("#qv-addcart-msg").fadeIn(700, function() {
                        var b = $(".qv-cartbtn", a);
                ...
                scaonError = function(XMLHttpRequest, textStatus) {
                    ...
                    data.message ? alert(data.message + "(" + data.status + "): " + data.description) : alert("Error : " + scafullMessagesFromErrors(data).join("; ") + "."), $(".qv-cartbtn").removeAttr("disabled")
                },
